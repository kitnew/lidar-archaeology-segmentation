...
This chapter analyses existing approaches to the interpretation and automated processing of LiDAR-derived terrain data in archaeology. The aim is not only to summarize previous work, but also to identify the methodological assumptions, advantages, and limitations of approaches that are relevant for the semantic segmentation of archaeological objects in airborne LiDAR data. The analysis therefore proceeds from traditional visual and GIS-based interpretation methods, through earlier semi-automatic approaches, to modern machine learning and deep learning methods. This structure provides the basis for the methodological choices made in the following chapters.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/aerial_photography_1.jpg}
        \caption{Aerial photographer operating a camera during World War I.}
        \label{fig:early_aerial_photography_photographer}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/aerial_photography_2.jpg}
        \caption{Aerial view of a French airfield.}
        \label{fig:early_aerial_photography_airfield}
    \end{subfigure}
    \caption{Historical examples of early aerial photography as a remote sensing data source. Source:~\cite{baumann_history_2014}.}
    \label{fig:early_aerial_photography}
\end{figure}
...
The conceptual difference between DTM, DFM, and DSM is illustrated in Figure~\ref{fig:dem_dfm_dsm}.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{figures/dem_dfm_dsm.png}
	\caption{Schematic distinction between Digital Terrain Model (DTM), Digital Feature Model (DFM), and Digital Surface Model (DSM). Source:~\cite{guyot_contribution_2021}.}
	\label{fig:dem_dfm_dsm}
\end{figure}
...
One of the main approaches was the visual inspection of raster visualizations derived from the terrain model. Besides the base elevation model itself, researchers employed various visualizations designed to enhance subtle micro-relief and improve the visibility of target archaeological features. The most commonly used visualizations include:
\begin{itemize}
...
Examples of several LiDAR-derived terrain visualizations used in archaeological interpretation are shown in Figure~\ref{fig:basic_visualizations}.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{figures/vats_example.jpg}
	\caption{Examples of LiDAR-derived terrain visualizations used for archaeological interpretation: (a) positive openness, (b) negative openness, (c) simple local relief model, (d) local dominance, (e) topographic position index, and (f) profile curvature. Source:~\cite{kokalj_standardizing_2025}.}
	\label{fig:basic_visualizations}
\end{figure}
